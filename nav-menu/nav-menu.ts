...
export class UINavMenu extends LitElement {
	...
	static styles: CSSResultGroup = css`
		...
		.nav-panel {
			...
			transition: opacity 0.2s ease, visibility 0.2s ease,
				transform 0.2s ease, left 0.22s ease;
			pointer-events: none;
		}
	...
	private _positionPanel(trigger: HTMLElement, width: number) {
		...
		left = Math.max(hostRect.left + g, Math.min(left, hostRect.right - width - g));
	...
	private _scheduleClose = () => {
		...
		this._closeTimer = setTimeout(() => {
			...
			this._triggers.forEach((t) => t.classList.remove('active'));
			setTimeout(() => {
