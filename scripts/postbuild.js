...
const componentNames = [
	'alert', 'aurora', 'button', 'card', 'details', 'dots',
	'draggable', 'droppable', 'gallery', 'globe', 'icon',
	'icon-selector', 'logo', 'meteors', 'nav-menu', 'ripple',
	'spinner', 'stat', 'tabs', 'text', 'code-block',
];
...
function organizeCss() {
	...
	for (const chunk of chunks) {
		...
		if (/^ui-/.test(trimmed)) {
			for (const name of componentNames) {
				if (trimmed.includes(`ui-${name}`) || trimmed.includes(`ui-bg-${name}`)) {
	...
	if (propertyDefs.length > 0) {
		writeFileSync(resolve(distDir, 'base.css'), `/*! @charged/ui base styles */\n${propertyDefs.join('\n')}`);
	}

	const withCss = componentNames.filter(name => componentStyles.get(name)?.length > 0);
	console.log(`CSS organized: ${withCss.length}/${componentNames.length} components have CSS`);
	if (propertyDefs.length > 0) console.log(`  base.css: ${propertyDefs.length} @property declarations`);
}
